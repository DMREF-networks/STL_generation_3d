...
def edges_to_adjacency_matrix(edges):
    ...
    edges (list of tuples): Either (node1, node2) pairs or
        (node1, node2, weight) triples. When a 3rd column is present it
        is stored in the adjacency matrix as a per-edge weight, which
        downstream multiplies the base beam diameter to produce
        variable-thickness beams. Without a 3rd column the entries are
        1.0 (uniform thickness, original behaviour).
